...
_IRQL_requires_max_(PASSIVE_LEVEL) NTSTATUS Acpi_EvaluateDsm(_In_ WDFDEVICE device,
	...
	if(!NT_SUCCESS(status)) {
        TraceEvents(TRACE_LEVEL_ERROR, TRACE_DEVICE, "[Device: 0x%p] IOCTL_ACPI_EVAL_METHOD for _DSM failed - %!STATUS!", device, status);
		goto Exit;
	}

	if(outputBuffer->Signature != ACPI_EVAL_OUTPUT_BUFFER_SIGNATURE) {
        TraceEvents(TRACE_LEVEL_ERROR, TRACE_DEVICE, "[Device: 0x%p] ACPI_EVAL_OUTPUT_BUFFER signature is incorrect", device);
...
NTSTATUS CrosECCreateDevice(_Inout_ PWDFDEVICE_INIT DeviceInit) {
	...
	if(SUCCEEDED_NTSTATUS(WdfFdoQueryForInterface(device, &GUID_ACPI_INTERFACE_STANDARD2,
		...
		if(SUCCEEDED_NTSTATUS(Acpi_EvaluateDsm(device, 0, &output))) {
			...
			if(output->Count == 1) {
				TraceEvents(TRACE_LEVEL_INFORMATION, TRACE_DEVICE, "Arg0 type is %d", output->Argument[0].Type);
